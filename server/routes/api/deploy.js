...
const deploy = require('../../interactors/deploy.js');
const slack = require('../../interactors/slack.js');
const crypto = require('../../interactors/crypto.js');
          ...
          deploy
            ...
            .then((result) => {
              slack.postSnippet(config.slack_token, {
                ...
                title: `${new Date().toString()} Staging Deploy Success`,
              });
            })
            .catch((err) => {
              slack.postSnippet(config.slack_token, {
                ...
                title: `${new Date().toString()} Staging Deploy Error`,
              });
            });
        }
        break;
  ...
  if (req.body.token === config.deployment_secret) {
    if (config.deployment_users.indexOf(req.body.user_name) !== -1) {
      ...
      if (req.body.text === 'production') {
        if (req.hostname === 'kappathetapi.com') {
          request.post({
            method: 'POST',
            uri: `https://staging.kappathetapi.com${req.originalUrl}`,
            body: req.body,
            json: true,
          });
          ...
          deploy
            ...
            .then((result) => {
              slack.postSnippet(config.slack_token, {
                ...
                content: deploy.formatResponse(result),
                title: `${new Date().toString()} Production Deploy Success`,
              });
            })
            .catch((err) => {
              slack.postSnippet(config.slack_token, {
                channels: req.body.channel_id,
                content: deploy.formatResponse(err),
                title: `${new Date().toString()} Production Deploy Error`,
              });
            });
        }
