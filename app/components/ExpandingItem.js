...
const Plus = ({ color, open }) =>
  h(PlusWrapper, [
    h(PlusLine, { color }),
    h(PlusLine, { color, vertical: !open }),
  ]);
...
export default class ExpandingItem extends React.Component {
  ...
  render() {
    return h(Wrapper, [
      h(Flexbox, { onClick: this.handleClick }, [
        h(Plus, {
          color: this.handlePlusColor,
          open: this.state.expanded,
        }),
        h(Header, { color: this.handleHeaderColor }, this.props.header),
      ]),
      h(
        Slider,
        { open: this.state.expanded },
        h(Body, { color: this.handleBodyColor }, this.props.body),
      ),
    ]);
  }
}
